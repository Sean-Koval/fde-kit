...
Fills the [AI security and governance review](../../../toolkit/ai-security-review.md) template. Judgment comes from [Production readiness for LLM systems](../../../skills/production-readiness.md) for guardrails, logging, and incident response; [Designing tool-using agents](../../../skills/agent-and-tool-design.md) for permission tiers and untrusted tool results; and [Retrieval and grounding](../../../skills/retrieval-and-grounding.md) for permissions-aware retrieval.
...
- Not as a substitute for the customer's review. The artifact prepares for it; the reviewer decides.

## Obtain first

- [ ] System version, the responsibility matrix, and a description of every hop from input to output to log.
- [ ] Data classification per input, retrieved source, tool result, output, and log; residency and retention requirements by policy identifier.
- [ ] The model provider's contractual terms as the customer holds them: data use, retention, region, subprocessors. Cite the contract; never summarize from memory.
- [ ] Tool inventory with permission tier, credential scope, and rate limits.
- [ ] Retrieval sources and how entitlements are enforced at query time.
- [ ] Logging: what is captured from prompts and outputs, where, who can read it, retention.
...
5. Record safety evaluation results by evaluation pack identifier and name the threat classes with no test coverage.
...
- Rules of thumb (a retention period, for example) are labeled starting heuristics, not industry standards; policy requirements are cited by identifier.
- Reference configurations, contracts, and logs by identifier. No credentials, keys, record contents, personal data, confidential document text, or working injection payloads in the review.
- Accepting residual risk belongs to the named risk/control owner and the customer's reviewer. The skill prepares; it does not approve.
