...
Fills the [responsibility matrix](../../../toolkit/responsibility-matrix.md) template. Judgment comes from [Human, software, and AI system design](../../../skills/ai-system-design.md) for the boundary, [Designing tool-using agents](../../../skills/agent-and-tool-design.md) for permission tiers and approval points, and [System patterns](../../../learning/system-patterns.md) for the least autonomous shape.
...
- Not without a validated workflow trace. Run [fde-workflow-trace](../fde-workflow-trace/SKILL.md) first; an imagined workflow yields imagined responsibilities.

## Obtain first

- [ ] The validated workflow trace, or an equivalent record with exceptions and controls.
- [ ] The workflow outcome, increment scope, and version identifier.
- [ ] Names and roles of the business outcome, technical service, and risk/control owners.
- [ ] Systems of record and the authoritative state for each responsibility.
...
- [ ] Tools the system will call, their reversibility, and the credentials available.
- [ ] Approvals already given, with approver and date.

## Procedure

1. Confirm the inputs. Owners and policy identifiers are the most often missing; ask before drafting.
2. Decompose the outcome into interpretation, validation, decision, action, and recovery (design guide, technique step 1). One row per responsibility; split any row that mixes interpretation with action.
3. Choose the shape with the least autonomy that reaches the outcome, using the decision rule in the agent guide's technique step 1 and the pattern notes in System patterns.
...
5. For every action row, assign the tool a permission tier and, for approval-tier actions, state what the approver will see (agent guide, technique steps 3 and 4). Treat tool results and retrieved content as untrusted data in the control column (step 8).
6. Fill required evidence, escalation trigger, and failure behavior for every AI or automated row (design guide, technique steps 4 and 5). A failure behavior returns the case to the existing queue or restores prior state; "retry" alone does not qualify.
7. Complete "Approval and open decisions". Record approvals only with an approver and date from the inputs; everything else is `open` with the evidence the approver will need.
...
- Rules of thumb (a step budget, for example) are labeled starting heuristics, not industry standards.
